...
export interface RenderCardOptions {
  stats: MonthlyStats | YearlyStats
  theme: string
  mode: 'monthly' | 'yearly'
  month: number
  year: number
  avatarBuffer: Buffer | null
  fonts: { regular: Buffer; bold: Buffer; extrabold: Buffer }
}
...
export async function renderCard({
  ...
  avatarBuffer,
  fonts,
}: RenderCardOptions): Promise<Buffer> {
  const { regular, bold, extrabold } = fonts
  ...
  if (mode === 'yearly') {
    ...
    const {
      commits, pullRequests, reposContributed, topLanguage, topLanguageColor,
      topRepo, name: fullName, totalStars, followers, dailyCommits,
      bestMonth, bestMonthCommits,
    } = yearlyStats
    ...
    const svg = await satori(yearlyCard, satoriOptions)
    const resvg = new Resvg(svg, { fitTo: { mode: 'width', value: 1080 } })
    return Buffer.from(new Uint8Array(resvg.render().asPng()))
  }
